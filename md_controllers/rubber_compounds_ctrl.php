...
class RubberCompoundsCtrl extends Ctrl {

  public function read_all($params = []) {
    if ($this->signed_in()) {
      $records = RubberCompounds::get(null, ['flat' => true]);
  ...
  public function read_one($params = []) {
    if ($this->signed_in()) {
      $record = RubberCompounds::get($params['id']);
  ...
  public function create_one($params = []) {
    ...
    if ($this->signed_in()) {
      $supplier = RubberCompounds::create($_BODY);
      if ( !$supplier->id ) {
